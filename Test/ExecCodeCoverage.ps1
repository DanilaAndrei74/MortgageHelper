...
$output = [string] (& dotnet test ../MortgageHelper.sln --collect:"XPlat Code Coverage" 2>&1)
...
Remove-Item -Recurse -Force $dir/TestCoverage/CoverageReport/

# To keep a history of the Code Coverage we need to use the argument: -historydir:SOME_DIRECTORY 
if (!(Test-Path -path $dir/TestCoverage/CoverageHistory)) {  
 New-Item -ItemType directory -Path $dir/TestCoverage/CoverageHistory
}
...
(& "$dir/TestCoverage/CoverageReport/index.html")
}